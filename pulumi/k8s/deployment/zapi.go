...
// ZAPIUtilizationStat returns the PromQL that reads a workload's in-flight
// request gauge as a per-replica utilization percentage, for use as
...
//
//   - zapi.ConcurrencyStatName qualifies the ambient stats prefix with the
...
// replica ceiling). The runtime publishes a raw count, so the division is what
// turns it into the 0-100 signal [UtilizationTrigger] expects: a replica
// holding perReplica requests reads as 100%. Averaging rather than summing is
...
func ZAPIUtilizationStat(e env.Env, namespace, name string, perReplica int) string {
	...
	return fmt.Sprintf("avg({__name__=%q}) * 100 / %d", metric, perReplica)
}
