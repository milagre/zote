...
func TestSpec_Validate_rejectsInvalidProfile(t *testing.T) {
	t.Parallel()

	s := &container.Spec{Profile: profile.Raw{}}
	if err := s.Validate(); err == nil || !strings.Contains(err.Error(), "profile") {
